...
def focus_and_squelch_with_injection(
    attn: torch.Tensor,        # Can be [B, HW, C] or [B, heads, HW, C] - ATTENTION SCORES (pre-softmax)
    region: torch.Tensor,      # [HW] values 0 or 1
    add_tokens: list[str],     # List of new tokens to inject
    pipe_tokenizer,            # Tokenizer for encoding new tokens
    boost: float = 0.5,        # Boost for injected tokens in target region
    suppress: float = 0.0,     # Suppression (not used for now)
) -> tuple[torch.Tensor, list[str]]:
    """
    Dynamically inject new tokens into attention scores in specified region
    """
    if not add_tokens:
        return attn, []
    ...
    if len(original_shape) == 3:
        B, HW, C = original_shape
        heads = 1
        attn_reshaped = attn.unsqueeze(1)  # Add heads dimension: [B, 1, HW, C]
    elif len(original_shape) == 4:
        B, heads, HW, C = original_shape
        attn_reshaped = attn
    else:
        raise ValueError(f"Expected 3D or 4D tensor, got shape {original_shape}")
    
    # Create attention scores for new tokens
    num_new_tokens = len(add_tokens)
    
    # Initialize new token scores at a reasonable baseline (not extremely negative)
    # We want them to start neutral and then get boosted in the target region
    new_token_scores = torch.full(
        (B, heads, HW, num_new_tokens), 
        0.0,  # Neutral baseline - will be boosted in target region
        device=device, 
        dtype=attn.dtype
    )
    
    # Create inside mask for the target region
    region_flat = region.flatten()
    if len(region_flat) != HW:
        region_flat = region_flat[:HW]
        if len(region_flat) < HW:
            padding = torch.zeros(HW - len(region_flat), device=device, dtype=region.dtype)
            region_flat = torch.cat([region_flat, padding])
    
    inside = region_flat.view(1, 1, HW, 1).to(device)
    inside = inside.expand(B, heads, HW, num_new_tokens)
    
    # Boost new tokens ONLY inside the target region
    boost_mask = inside.float()
    new_token_scores = new_token_scores + boost * boost_mask
    
    # Apply suppression to existing tokens in the target region
    if suppress > 0:
        suppress_mask = region_flat.view(1, 1, HW, 1).to(device)
        suppress_mask = suppress_mask.expand(B, heads, HW, C).float()
        attn_reshaped = attn_reshaped - suppress * suppress_mask
    
    # Concatenate original scores with new token scores
    expanded_scores = torch.cat([attn_reshaped, new_token_scores], dim=-1)
    
    
    # Successfully injected new tokens for attention expansion
    # Return to original shape structure if needed
    if len(original_shape) == 3:
        expanded_scores = expanded_scores.squeeze(1)
    
    return expanded_scores, add_tokens

def focus_and_squelch(
    attn: torch.Tensor,        # Can be [B, HW, C] or [B, heads, HW, C]
    region: torch.Tensor,      # [HW] values 0 or 1
    add_idxs: list[int],
    boost: float  = 0.5,       # Gentler score boost (now applied to raw scores)
    suppress: float = 0.2,     # Very strong suppression of other tokens
) -> torch.Tensor:
    """
    Attention-conserving addition:
    Inside the mask:
     - boost add_idxs by (1+boost) 
     - suppress *all other* tokens by (1-suppress)
    Outside the mask:
     - gently reduce add_idxs by (1-gentle_reduce) to conserve attention
    ...
    # Ensure region has the right shape [HW] -> [1, 1, HW, 1]
    region_flat = region.flatten()
    if len(region_flat) != HW:
        # If region doesn't match HW, it might need reshaping
        region_flat = region_flat[:HW]  # Truncate if too long
        if len(region_flat) < HW:
            # Pad if too short
            padding = torch.zeros(HW - len(region_flat), device=device, dtype=region.dtype)
            region_flat = torch.cat([region_flat, padding])
    
    # Create inside and outside masks for aggressive contrast
    inside = region_flat.view(1, 1, HW, 1).to(device)
    inside = inside.expand(B, heads, HW, 1)
    outside = 1.0 - inside  # Outside mask for suppression
    
    # Strong suppression outside the box to create sharp contrast
    outside_suppress = 0.7  # Strongly suppress target tokens outside box

    out = attn_reshaped.clone()
    
    # Safety check: ensure no NaN in inputs
    out = torch.nan_to_num(out, nan=0.0, posinf=1.0, neginf=0.0)
    
    # Process each token channel - AGGRESSIVE LOCALIZATION
    for t in range(C):
        if t >= out.shape[-1]:  # Safety check
            break
            
        chan = out[..., t:t+1]  # Shape: [B, heads, HW, 1]
        
        if t in add_idxs:
            # AGGRESSIVE: Set very high attention scores inside the box
            original_max = chan.max().item()
            original_mean = chan.mean().item()
            
            # Conservative additive boost only
            boost_applied = boost * inside
            chan = chan + boost_applied
            
            final_max = chan.max().item()
            final_mean = chan.mean().item()
            
            # Debug removed - we know the modifications are working
            
            # NO outside suppression to prevent global color tint
        else:
            # Leave other tokens completely unchanged everywhere
            pass
            
        out[..., t:t+1] = chan
    
    # Safety check after modifications
    out = torch.nan_to_num(out, nan=0.0, posinf=1.0, neginf=0.0)
    ...
    # SELECTIVE NORMALIZATION: DISABLED FOR FINAL TEST
    # Testing raw 200x boost effect without normalization interference
    
    # inside_expanded = inside.expand_as(out)  # Shape: [B, heads, HW, C]  
    # inside_sum = (out * inside_expanded).sum(dim=-1, keepdim=True)
    # inside_sum = torch.clamp(inside_sum, min=1e-6)
    # normalized_inside = torch.where(
    #     inside_sum > 1e-6,
    #     out * inside_expanded / inside_sum,
    #     out * inside_expanded
    # )
    # out = out * (1 - inside_expanded) + normalized_inside
    ...
    if len(original_shape) == 3:
        out = out.squeeze(1)  # Remove heads dimension: [B, HW, C]
    
    return out


def remove_and_boost(
    attn: torch.Tensor,        # Can be [B, HW, C] or [B, heads, HW, C]
    region: torch.Tensor,      # [HW] values 0 or 1
    ...
    """
    Attention-conserving removal:
    Inside the mask:
     - suppress remove_idxs by (1-suppress)
     - boost *all other* tokens by (1+boost_all)
    Outside the mask:
     - gently boost remove_idxs by (1+gentle_boost) to conserve attention
     - leave other tokens unchanged
    """
    original_shape = attn.shape
    device = attn.device
    ...
    # Create inside mask only - NO OUTSIDE MODIFICATIONS
    inside = region_flat.view(1, 1, HW, 1).to(device)
    inside = inside.expand(B, heads, HW, 1)

    out = attn_reshaped.clone()
    
    # Process each token channel - ONLY MODIFY INSIDE THE BOX
    for t in range(C):
        if t >= out.shape[-1]:  # Safety check
            break
        ...
        if t in remove_idxs:
            # Suppress removal targets ONLY inside the region
            chan = chan * (1 - suppress * inside)
        else:
            # Boost other tokens ONLY inside the region
            chan = chan * (1 + boost_all * inside)
            
        out[..., t:t+1] = chan

    # Clamp only - DISABLE NORMALIZATION FOR TESTING
    out = torch.nan_to_num(out, nan=0.0, posinf=1.0, neginf=0.0)
    out = out.clamp(min=0.0)
    
    # NORMALIZATION DISABLED - Test if this causes the red tint
    # out_sum = out.sum(dim=-1, keepdim=True)
    # out_sum = torch.clamp(out_sum, min=1e-8)  # Prevent division by zero
    # out = out / out_sum
